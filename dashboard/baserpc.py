...
class DashboardRPC:
    ...
    async def get_variables(self):
        ...
        returning = {
            ...
            "botavatar": self.bot.user.display_avatar.url,
    ...
    async def get_users_servers(self, userid: int):
        ...
        async for guild in AsyncIter(self.bot.guilds, steps=1300):
            ic = (guild.icon or self.bot.user.display_avatar)
            sgd = {
    ...
    async def get_server(self, userid: int, serverid: int):
       ...
       #  region = " ".join(parts)
        ...
        guild_data = {
            ...
            "id": guild.id,
            "owner": escape(str(guild.owner)),
            "icon": getattr(guild.icon, 'url', "https://cdn.discordapp.com/embed/avatars/1."),
            "animated": getattr(guild.icon, 'is_animates', lambda: False)(),
