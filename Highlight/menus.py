...
class ChannelShowMenu(discord.ui.View):
    def __init__(self, ctx: commands.Context, highlight_data):
        self._ctx = ctx
    ...
    def handle_request(self, selected_option):
        ...
        embed = discord.Embed(
            title = f"Current highlights for {self._ctx.author.name} in {underline(self._objects[int(selected_option)].name)}",
